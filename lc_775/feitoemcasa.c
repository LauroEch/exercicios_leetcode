...
void inversao( no_t *raiz, int valor, long long *global ){
    if( raiz == NULL ){
        return;
    }
    
    if( valor < raiz->valor ){
        *global += 1 + Tamanho( raiz->dir );
        inversao( raiz->esq, valor, global );
    }else{
        inversao( raiz->dir,valor,global );
    }
}
/*
====================
