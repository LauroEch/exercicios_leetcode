...
void inversao(no_t *raiz, int valor, int *global,int *local){
    if(raiz == NULL){
        return;
    }
    if(valor > raiz->valor){
        (*local)++;
        inversao(raiz->dir,valor,global,local);
    }else if(valor < raiz->valor){
        (*global)++;
        inversao(raiz->esq,valor,global,local);
        return;
    }
}
/*
...
bool isIdealPermutation( int *nums, int numsSize ) {
    ...
    for(int i = 0;i < numsSize;i++){
        ...
        inversao(raiz,nums[i],&tempGlobal,&tempLocal);

        if(tempGlobal > global){
            global = tempGlobal;
        }
        if(tempLocal > local){
            local = tempLocal;
