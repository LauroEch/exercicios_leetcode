...
int **fourSum( int *nums, int numsSize, int target,
			   int *returnSize, int **returnColumnSizes ) {
                ...
                for( i = 0; i < numsSize - 3 ;i++ ){
                    ...
                    for( j = i + 1; j < numsSize - 2 ;j++ ){
                        ...
                        l = numsSize - 1;
                        while ( k != l && l >= k ) {
                            soma = ( long long )nums[i] + nums[j] + nums[k] + nums[l];
                            if ( soma == target ) {
                                retorno[qtdQuadruplas] = malloc( sizeof( int ) * 4 );
                                retorno[qtdQuadruplas][0] = nums[i];
                                retorno[qtdQuadruplas][1] = nums[j];
                                retorno[qtdQuadruplas][2] = nums[k];
                                retorno[qtdQuadruplas][3] = nums[l];
                                qtdQuadruplas++;
                                l--;
                                k++;
                ...
                *returnColumnSizes = malloc( sizeof( int ) * qtdQuadruplas );

                for( int i = 0; i < qtdQuadruplas ;i++ ){
                    ( *returnColumnSizes )[i] = 4;
                }

                retorno = realloc( retorno,sizeof( int* ) * qtdQuadruplas );
                *returnSize = qtdQuadruplas;
